...
public class SFObjectTablePrinter implements Printable, Pageable, ISFPrintPreviewable {
  protected SFObjectTable table = null;
  ...
  public SFObjectTablePrinter(SFObjectTable _table, PageFormat _pageFormat) {
    table = _table;
    pageFormat = _pageFormat;
    initTableMeasurements();
  }

  public SFObjectTablePrinter(SFObjectTable _table, PageFormat _pageFormat, String _headerTitle, String _headerDateRange, String _headerCreateDTS) {
    table = _table;
    pageFormat = _pageFormat;
  ...
  public static void printTable(SFObjectTable _table, boolean _forceLandscape) {
    printTable(_table, _forceLandscape, false);
  }

  public static void printTable(SFObjectTable _table, boolean _forceLandscape, boolean _showPreview) {
    printTable(_table, _forceLandscape, false);
  }

  public static void printTable(final SFObjectTable _table, final boolean _forceLandscape, final boolean _showPreview,
  ...
  try {
    ...
    SFObjectTablePrinter tmpPrinter;
    if(_headerTitle != null && _headerDateRange != null && _headerCreateDTS != null)
      tmpPrinter = new SFObjectTablePrinter(_table, pageFormat, _headerTitle, _headerDateRange, _headerCreateDTS);
    else
      tmpPrinter = new SFObjectTablePrinter(_table, pageFormat);
  ...
  public void initTableMeasurements() {
    ...
    int simpleRowHeight = 2 * PrintingUtility.itemSpacing + fontData.plainHeight + PrintingUtility.gridWidth;
    int usedHeight = 0;
    int tmpPageCount = 0;
    int tmpMaxHeight = 0;
    int tmpInt = 0;
  ...
  tmpColWidth += tmpInt;
      tmpInt = PrintingUtility.getMultiLineStrHeight(table.getColumnName(i), fontData.plainHeight, tmpInt, fm);
  ...
  tmpMaxHeight = tmpInt;
    ...
    //adjust displaimer
    //tmpMaxHeight = PrintingUtility.getMultiLineStrHeight(PrintingUtility.EMAIL_DISCLAIMER, fontData.plainHeight, paperWidth, fm);
    //intPageHeight -= (tmpMaxHeight + PrintingUtility.itemSpacing);
    //take out header.
    if(header != null)
      tmpMaxHeight = fontData.headerHeight + 3 * PrintingUtility.itemSpacing + 2 * fontData.boldHeight;
    intPageHeight -= tmpMaxHeight;
    //table data.
  ...
  for(int j = 0; j < colCount; j++) {
    tmpInt = PrintingUtility.getMultiLineStrHeight(table.getStringValueAt(i, j, false), fontData.plainHeight, colWidths[j] - 2 * PrintingUtility.gridWidth - 2 * PrintingUtility.itemSpacing, fm);
    if(tmpInt > tmpMaxHeight)
      tmpMaxHeight = tmpInt;
  }
  ...
  int[] pageBreaks = PrintingUtility.getMultiLineStrPageBreaks(fontData.plainHeight, tmpMaxHeight, usedHeight + 2 * PrintingUtility.itemSpacing + PrintingUtility.gridWidth, intPageHeight);
  if(pageBreaks.length < 2)
    ...
    usedHeight = pageBreaks[pageBreaks.length - 1] + PrintingUtility.itemSpacing + PrintingUtility.gridWidth;
  }
      }
    } //end for
    if(usedHeight > 0)
  ...
  public int paintTable(Graphics g
    ...
    //int disclaimerHeight = PrintingUtility.getMultiLineStrHeight(PrintingUtility.EMAIL_DISCLAIMER, fontData.plainHeight, intPageWidth, g2.getFontMetrics(PrintingUtility.tablePlainFont));
    //pageHeight -= (disclaimerHeight + PrintingUtility.itemSpacing);
    //take out col headers
    ...
    int colCount = table.getColumnCount();
    int baseRowHeight = table.getRowHeight();
    int simpleRowHeight = fontData.plainHeight + 2 * PrintingUtility.itemSpacing + PrintingUtility.gridWidth;
    int totalColumnWidth = (int)(columnScale * (double)table.getColumnModel().getTotalColumnWidth());
    //translate
    g2.translate(tx, ty);
  ...
  //g.drawImage(tmpImg.getImage(), intPageWidth - 262, usedHeight, 262, 40, null);
      //}
      //title
      g.setClip(usedWidth, usedHeight, intPageWidth - usedWidth, fontData.headerHeight + PrintingUtility.itemSpacing);
      if(PrintingUtility.checkLineForPage(usedHeight, intPageHeight, 0))
  g.drawString(header.titleString, usedWidth, usedHeight + fontData.headerHeight - fontData.headerDesent);
      usedHeight += fontData.headerHeight + PrintingUtility.itemSpacing;
      //created
      g.setFont(PrintingUtility.tableBoldFont);
      g.setClip(usedWidth, usedHeight, intPageWidth - usedWidth, fontData.boldHeight + PrintingUtility.itemSpacing);
      if(PrintingUtility.checkLineForPage(usedHeight, intPageHeight, 0))
  g.drawString(header.createdDTSStr, usedWidth, usedHeight + fontData.boldHeight - fontData.boldDesent);
      usedHeight += fontData.boldHeight + PrintingUtility.itemSpacing;
      //date range.
      g.setClip(usedWidth, usedHeight, intPageWidth - usedWidth, fontData.boldHeight + PrintingUtility.itemSpacing);
      if(PrintingUtility.checkLineForPage(usedHeight, intPageHeight, 0))
  g.drawString(header.dateRangeStr, usedWidth, usedHeight + fontData.boldHeight - fontData.boldDesent);
      usedHeight += fontData.boldHeight + PrintingUtility.itemSpacing;
    }
    //paint columns.
    g2.setFont(PrintingUtility.tablePlainFont);
    ...
    //for(int i = 0; i < colCount; i++) {
      ...
      //tmpInt = PrintingUtility.getMultiLineStrHeight(table.getColumnName(i), fontData.plainHeight, table.getColumnModel().getColumn(i).getWidth(), g2.getFontMetrics());
  ...
  //tmpUsedHeight = tmpInt;
    ...
    for(int i = startCol; i < endCol; i++) {//colCount; i++) {
      ...
      g2.setClip(PrintingUtility.getMinPageWidthClip(usedWidth, intPageWidth, 0), usedHeight, PrintingUtility.getMaxPageWidthClip(usedWidth, colWidths[i], intPageWidth, 0), colHeaderHeight);// tmpMaxHeight + 2 * PrintingUtility.itemSpacing + 2 * PrintingUtility.gridWidth);
  ...
  g2.drawLine(usedWidth, usedHeight, usedWidth, usedHeight + colHeaderHeight);//tmpUsedHeight + 2 * PrintingUtility.itemSpacing + 2 * PrintingUtility.gridWidth);
      ...
      g2.drawLine(usedWidth + colWidths[i], usedHeight, usedWidth + colWidths[i], usedHeight + colHeaderHeight);// + tmpUsedHeight + 2 * PrintingUtility.itemSpacing + 2 * PrintingUtility.gridWidth);
      g2.drawLine(usedWidth, usedHeight + colHeaderHeight, usedWidth + colWidths[i], usedHeight + colHeaderHeight);
      PrintingUtility.paintMultiLineStr(g2, table.getColumnName(i), usedWidth + PrintingUtility.gridWidth + PrintingUtility.itemSpacing, usedHeight + PrintingUtility.gridWidth + PrintingUtility.itemSpacing,
    colWidths[i] - 2 * PrintingUtility.gridWidth - 2 * PrintingUtility.itemSpacing, intPageHeight + colHeaderHeight, intPageWidth, 0, 0);
      //}
  ...
  //if(PrintingUtility.checkLineForPage(usedHeight + simpleRowHeight, intPageHeight, currentRowPage, usedWidth, colWidths[i], intPageWidth, currentColPage)) {
    tmpStr = table.getPrintValueAt(j, i);
    g2.setClip(PrintingUtility.getMinPageWidthClip(usedWidth, intPageWidth, 0), PrintingUtility.getMinPageHeightClip(usedHeight, intPageHeight, 0), PrintingUtility.getMaxPageWidthClip(usedWidth, colWidths[i], intPageWidth, 0), PrintingUtility.getMaxPageHeightClip(usedHeight, rowHeights[j], intPageHeight, 0));//tmpUsedHeight + 2 * PrintingUtility.itemSpacing + PrintingUtility.gridWidth);
    if(tmpStr != null) {
      if(rowHeights[j] > simpleRowHeight) {
        tmpInt = PrintingUtility.paintPageBreakMultiLineStr(g2, tmpStr, usedWidth + PrintingUtility.gridWidth + PrintingUtility.itemSpacing, usedHeight + PrintingUtility.gridWidth + PrintingUtility.itemSpacing - tmpAdjustment,
    colWidths[i] - 2 * PrintingUtility.gridWidth - 2 * PrintingUtility.itemSpacing, intPageHeight, 0, usedHeight);
        //tmpInt = PrintingUtility.paintMultiLineStr(g2, tmpStr, usedWidth + PrintingUtility.gridWidth + PrintingUtility.itemSpacing, usedHeight + PrintingUtility.gridWidth + PrintingUtility.itemSpacing,
    //colWidths[i] - 2 * PrintingUtility.gridWidth - 2 * PrintingUtility.itemSpacing, intPageHeight, intPageWidth, 0, 0);//currentRowPage, currentColPage);
        //tmpInt = PrintingUtility.getMultiLineStrHeight(tmpInt, fontData.plainHeight);
        //if(tmpInt > tmpUsedHeight)
    //tmpUsedHeight = tmpInt;
      }
      else {
        PrintingUtility.paintElipseStr(g2, tmpStr, usedWidth + 2 * PrintingUtility.gridWidth + PrintingUtility.itemSpacing, usedHeight + fontData.plainHeight - fontData.plainDesent + PrintingUtility.itemSpacing,
      colWidths[i] - 2 * PrintingUtility.gridWidth - 2 * PrintingUtility.itemSpacing, usedHeight, simpleRowHeight);
      ...
      table.printValueAt(g2, j, i, usedWidth + 2 * PrintingUtility.gridWidth + PrintingUtility.itemSpacing, usedHeight + PrintingUtility.itemSpacing, colWidths[i] - 2 * PrintingUtility.gridWidth - 2 * PrintingUtility.itemSpacing, rowHeights[j] - 2 * PrintingUtility.gridWidth);
    }
    g2.setClip(PrintingUtility.getMinPageWidthClip(usedWidth, intPageWidth, 0), PrintingUtility.getMinPageHeightClip(usedHeight, intPageHeight, 0), PrintingUtility.getMaxPageWidthClip(usedWidth, colWidths[i], intPageWidth, 0), PrintingUtility.getMaxPageHeightClip(usedHeight, rowHeights[j], intPageHeight, 0));//tmpUsedHeight + 2 * PrintingUtility.itemSpacing + PrintingUtility.gridWidth);
    if(i == startCol)
      g2.drawLine(usedWidth, usedHeight, usedWidth, usedHeight + rowHeights[j] - tmpAdjustment);//tmpUsedHeight + 2 * PrintingUtility.itemSpacing + PrintingUtility.gridWidth);
    g2.drawLine(usedWidth + colWidths[i], usedHeight, usedWidth + colWidths[i], usedHeight + rowHeights[j] - tmpAdjustment);//tmpUsedHeight + 2 * PrintingUtility.itemSpacing + 2 * PrintingUtility.gridWidth);
    g2.drawLine(usedWidth, usedHeight + rowHeights[j] - tmpAdjustment, usedWidth + colWidths[i], usedHeight + rowHeights[j] - tmpAdjustment);//tmpUsedHeight + 2 * PrintingUtility.itemSpacing + PrintingUtility.gridWidth);
  //}
  usedWidth+=colWidths[i];
      }
  ...
  //intPageWidth - 1, intPageHeight + disclaimerHeight + PrintingUtility.itemSpacing, intPageWidth, 0, 0);//currentRowPage, currentColPage);
    ...
    g2.setFont(PrintingUtility.tablePlainFont);
    g2.setColor(PrintingUtility.blackColor);
    g2.setClip(0, intPageHeight + 2 * PrintingUtility.itemSpacing, intPageWidth, fontData.plainHeight + PrintingUtility.lineSpacing);
    g2.drawString("Page " + (pageIndex + 1) + " of " + totalPageCount + "  - printed on " + DataUtility.dateToDisplayString(currentDateTime)
      , (intPageWidth / 2 - 70)
      , intPageHeight + 2 * PrintingUtility.itemSpacing + fontData.plainHeight - fontData.plainDesent);
    //end
