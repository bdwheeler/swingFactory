...
public class SFTableTree extends SFTable {
  
	protected SFTableTreeRenderer renderer;
  ...
  private SFTableTreeModel model;
  private ListToTreeSelectionModelWrapper selectionWrapper;

  public SFTableTree(ISFTableTreeModel _model, SFTableColumn[] headers) {
    renderer = new SFTableTreeRenderer(this, _model);
    model = new SFTableTreeModel(_model, renderer);
  ...
  public SFTableTreeModel getSFModel() {
    return model;
  }
  ...
  public SFTableTreeRenderer getTreeCellRenderer() {
    return renderer;
  }
  ...
  public class SFTableTreeSortAdapter extends SFTableSortAdapter
  {
    public SFTableTreeSortAdapter(SFTableTree table) {
      super(table);
    }
    ...
    public int compareRowsByColumn(int row1, int row2, int column) {
      if(column == 0) {
        ...
        //int childCnt1 = ((SFTableTreeModel)tableModel).getChildCount(row1);
        //int childCnt2 = ((SFTableTreeModel)tableModel).getChildCount(row2);
    ...
    public void sort(Object sender) {
      ...
      for(int i = 0; i < indexes.length; i++) {
        int childCnt = 0;
        if(((SFTableTreeModel)tableModel).isRowExpanded(i))
          childCnt += ((SFTableTreeModel)tableModel).getChildCount(i);
      ...
      for(int i = 0; i < parentArray.length; i++) {
        ...
        cntr++;
        int childCnt = 0;
        if(((SFTableTreeModel)tableModel).isRowExpanded(parentArray[i]))
          childCnt += ((SFTableTreeModel)tableModel).getChildCount(parentArray[i]);
        if(childCnt > 0) {
